    ...
    def initiate_validation(self):
        try:
            ...
            for subfolder_path in subfolders:
                ...
                for image_file in files:
                    ...
                    if self.is_corrupt_image(image_path):
                        ...
                        # Move corrupted images to the corrupted_image_filepath folder
                        image_filename = os.path.basename(image_path)
                        destination_path = os.path.join(self.data_validation_config.corrupted_image_filepath, image_filename)
                        os.makedirs(os.path.dirname(destination_path), exist_ok=True)
                        os.rename(image_path, destination_path)

                    # Add valid images to the report
                    validation_report["valid_images"].append(image_path)
            ...
            for image_path in valid_images:
                image_filename = os.path.basename(image_path)
                destination_path = os.path.join(self.data_validation_config.valid_image_filepath, image_filename)
                os.makedirs(os.path.dirname(destination_path), exist_ok=True)
                os.rename(image_path, destination_path)
